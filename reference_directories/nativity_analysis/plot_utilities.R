...
make_fertility_table <- function(merged_df, 
  ...
  table_df <- table_df %>%
    ...
    ungroup() %>%
    # Filter: for total type show only total category (lowercase), for groups type show only non-total
    filter((type == "total" & category == "total") | (type == "groups" & category != "total")) %>%
